...
class SinglePersonSync {
  ...
  constructor(params: { buid: string, crudOperation: CrudOperation, configPath?: string }) {
    ...
    this.config = configManager.loadConfig(params.configPath);
    ...
    if (this.config.dataSource.fieldsToKeep) {
      responseFilter = new PersonDataSourceResponseStreamFilter({ fieldsToKeep: this.config.dataSource.fieldsToKeep });
    }
