...
import { ResponseProcessor, PersonDataSourceResponseStreamFilter } from '../stream/PersonDataSourceResponseStreamFilter';
  ...
  async fetchRaw(): Promise<any[]> {
    try {
      ...
      const response = await this.apiClient.get<{ response: any[] }>({
        url: this.config.dataSource.fetchPersonsEndpoint + (this.params.buid ? `?buid=${this.params.buid}` : ''),
