# TODO: This README has been copied to the integration-huron-person-fargate repo, where its level of detail is more appropriate. The version of it here should be simplified and retain just enough visually to make it clear the the syncing process MAY be involved in an outer chunking operation that it is agnostic to, and that if so, there is a post-processing merging step that happens after all chunks have completed.
