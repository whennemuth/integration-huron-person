...
/**
 * Configuration for PersonDataSourceResponseStreamFilter
 */
export interface PersonDataSourceResponseStreamFilterConfig {
  fieldsToKeep: string[];
  customFilterCase?: (source: any, target?: any) => void
}

/**
 * Response stream filter specifically designed for PersonDataSource operations.
 ...
 * @example
 * ```typescript
 * const filter = new PersonDataSourceResponseStreamFilter({
 ...
 * ```
 */
export class PersonDataSourceResponseStreamFilter implements ResponseProcessor {

  constructor(private config: PersonDataSourceResponseStreamFilterConfig) {}
  ...
  async processResponse<T>(response: AxiosResponse<T>): Promise<AxiosResponse<T>> {
    return new Promise((resolve, reject) => {
      const filteredPersons: any[] = [];
      const { fieldsToKeep, customFilterCase } = this.config;
      
      (response.data as any)
        .pipe(new JsonParser({ extractPath: 'response[*]' }))  // Extract person objects from response array
        .pipe(new JsonFieldFilter(fieldsToKeep, customFilterCase))   // Filter fields from each person
        .on('data', (filteredPerson: any) => {
          filteredPersons.push(filteredPerson);  // Collect filtered persons
        })
        .on('end', () => {
          // Replace response data with filtered results
          (response as any).data = { response: filteredPersons };
          resolve(response);
        })
        .on('error', reject);
    });
