...
3. **PersonDataSourceResponseStreamFilter** - Orchestrates the streaming pipeline for API responses

```mermaid
sequenceDiagram
    participant HTTP as HTTP Response Stream
    participant PSF as PersonDataSourceResponseStreamFilter
    ...
    loop Each chunk
        ...
        JP->>JP: Extract objects (persons)<br>as they complete
        JP->>JF: Stream each object (person)<br>immediately
        JF->>JF: Filter person object<br>(reduce memory)
...
const customFilter = (source: any, target: any) => {
  ...
  // Modify target based on source data
...
```typescript
import { PersonDataSourceResponseStreamFilter } from './PersonDataSourceResponseStreamFilter';
...
The `PersonDataSourceResponseStreamFilterConfig` interface supports:
