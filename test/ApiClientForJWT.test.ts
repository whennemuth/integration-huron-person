...
describe('ApiClientForJWT', () => {
  ...
  describe('HTTP methods', () => {
    ...
    it('should handle GET requests', async () => {
      ...
      const result = await apiClient.get({ url: '/users/1', responseFilter: new PersonDataSourceResponseStreamFilter({ fieldsToKeep: ['id'] }) });
  ...
  describe('error handling', () => {
    it('should propagate network errors', async () => {
      ...
      await expect(apiClient.get({ url: '/test', responseFilter: new PersonDataSourceResponseStreamFilter({ fieldsToKeep: ['id'] }) })).rejects.toThrow('Network Error');
    });
    ...
    it('should handle HTTP error responses', async () => {
      ...
      await expect(apiClient.get({ url: '/nonexistent', responseFilter: new PersonDataSourceResponseStreamFilter({ fieldsToKeep: ['id'] }) })).rejects.toEqual(httpError);
    });
  });
});