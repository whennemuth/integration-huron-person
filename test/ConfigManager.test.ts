...
describe('ConfigManager', () => {
  ...
  describe('getEnvironmentConfig', () => {
    ...
    it('should override DataTarget JWT configuration', () => {
      ...
      expect(result.dataTarget?.endpointConfig?.username).toBe('prod-user');
      expect(result.dataTarget?.endpointConfig?.password).toBe('prod-password');
      expect(result.dataTarget?.endpointConfig?.authTokenUrl).toBe('/prod/auth/token');
    });
    ...
    it('should handle partial overrides correctly', () => {
      ...
      expect(result.dataTarget?.endpointConfig?.username).toBe('new-username');
      ...
      expect(result.dataTarget?.pushPersonsEndpoint).toBe('/api/v1/persons/batch');
    });
    ...
    it('should preserve existing endpointConfig when adding overrides', () => {
      ...
      expect(result.dataTarget?.endpointConfig?.password).toBe('override-password');
