...
import { HuronPersonDataTarget, PersonPushRequest, PersonPushResponse } from '../src/PersonDataTarget';
...
class MockApiClient implements IApiClient {
  ...
  async get<T = any>(url: string, params?: any): Promise<{ data: T; status: number; statusText: string; headers: {}; config: any; }> {
...
describe('HuronPersonDataTarget', () => {
    ...
    it('should prioritize endpointConfig timeout over integration timeout', () => {
      const configWithBothTimeouts: Config = {
        ...
        dataTarget: {
          ...
          pushPersonsEndpoint: '/api/v1/persons'
        },
        integration: {
  ...
  describe('pushOne', () => {
    ...
    it('should handle single DELETE operation', async () => {
      ...
      const fieldSet = createFieldSet({ id: 'person-1' });
    ...
    it('should handle unsuccessful API response', async () => {
      const mockResponse: PersonPushResponse = {
        ...
        errors: ['Email is required', 'Invalid department']
      };
      mockApiClient = new MockApiClient(mockResponse);
      (dataTarget as any).apiClient = mockApiClient;
  ...
  describe('pushAll', () => {
    it('should successfully push batch operations', async () => {
      const mockResponse: PersonPushResponse[] = [
        { success: true, id: 'person-1', message: 'Created' },
        { success: true, id: 'person-2', message: 'Updated' },
        { success: true, id: 'person-3', message: 'Deleted' }
      ];
      mockApiClient = new MockApiClient(mockResponse);
      (dataTarget as any).apiClient = mockApiClient;
    ...
    it('should handle mixed success/failure batch operations', async () => {
      const mockResponse: PersonPushResponse[] = [
        { success: true, id: 'person-1', message: 'Created' },
        { success: false, id: 'person-2', message: 'Validation error', errors: ['Invalid email'] },
        { success: true, id: 'person-3', message: 'Updated' }
      ];
      mockApiClient = new MockApiClient(mockResponse);
      (dataTarget as any).apiClient = mockApiClient;
